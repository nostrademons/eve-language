...
FileLine : SequenceUnpack               { FileLine (Definition $1) $ pos $1 }
         | 'import' DottedIdent         { FileLine (Import $ reverse $2) $ pos $1 }
         | 'export' VarList             { FileLine (Export $ reverse $2) $ pos $1 }
         | DefDecl                      { FileLine (Definition $1) $ pos $1 }

DefDecl     : 'def' VAR '(' VarArgList ')' TypeDecl ':' DefBody
    { let (doc, lines, body) = $8 in DefLine (Def (extractVar $2) $4 doc $6 lines body) $ pos $1 }

DocString   : {- Empty -}       { "" }
            | STR EOL           { $1 }

DefBody : Expr                                      { ("", [], $1) }
        | EOL INDENT DocString DefLineList DefExpr DEDENT   { ($3, reverse $ tail $4, $5) }

DefLine : SequenceUnpack EOL        { $1 }
        | DefDecl EOL               { $1 }
...
DefLineList     : {- empty -}           { [] }
                | DefLineList DefLine   { $2 : $1 }

MultiLineExpr   : 'cond' ':' EOL INDENT CondClauseList DEDENT   { untypedExpr (Cond $ reverse $5) $ pos $1 }

SequenceUnpack : VAR '=' Expr          { DefLine (Binding (extractVar $1) $3) $ pos $2 }
               | VarList '=' Expr      { DefLine (SequenceUnpack (reverse $1) $3) $ pos $2 }
...
Expr : Operand             { $1 }
     | '-' Expr %prec NEG  { funcall (pos $1) "sub" [untypedExpr (Literal $ LitInt 0) $ pos $1, $2] }
     ...
     | Expr '->' Expr   { untypedExpr (Funcall $3 [$1]) $ pos $2 }
     | 'if' Expr 'then' Expr 'else' Expr
       { untypedExpr (Cond [($2, $4), (untypedExpr (Literal $ LitBool True) $ pos $1, $6)]) $ pos $1 }
...
-}
            ...
            | Operand '[' Expr ']'              { funcall (pos $2) "get" [$1, $3] }
            | Operand '[' Expr ':' Expr ']'     { funcall (pos $2) "slice" [$1, $3, $5] }
            | Operand '(' ExprList ')'          { untypedExpr (Funcall $1 (reverse $3)) $ pos $2 }
            | Operand '(' ')'                   { untypedExpr (Funcall $1 []) $ pos $2 }
...
DefaultDecl : {- Empty -}           { Nothing }
            | '=' Operand           { Just $2 }
ArgList     : {- empty -}                          { [] }
            | VAR DefaultDecl TypeDecl             { [Arg (extractVar $1) $2 $3] }
            | ArgList ',' VAR DefaultDecl TypeDecl { (Arg (extractVar $3) $4 $5) : $1 }
VarArgList  : ArgList                  { ArgList $1 Nothing }
